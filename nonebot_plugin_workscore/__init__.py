...
@ces.args_parser
async def parse(state: T_State = State()):
    resourcepath = dirname(__file__) + "/resource"
    if not os.path.exists(resourcepath):
        print("构建资源目录")
        os.mkdir(resourcepath)
        print("下载资源文件")
        open(dirname(__file__) + "/resource/" + "shuang.png", 'wb').write(
            requests.get("https://cdn.jsdelivr.net/gh/yzyyz1387/blogimages/shuang.png").content)
        open(dirname(__file__) + "/resource/" + "shuangboom.png", 'wb').write(
            requests.get("https://cdn.jsdelivr.net/gh/yzyyz1387/blogimages/shuangboom.png").content)
        open(dirname(__file__) + "/resource/" + "yiban.png", 'wb').write(
            requests.get("https://cdn.jsdelivr.net/gh/yzyyz1387/blogimages/yiban.png").content)
        open(dirname(__file__) + "/resource/" + "cjrh.png", 'wb').write(
            requests.get("https://cdn.jsdelivr.net/gh/yzyyz1387/blogimages/cjrh.png").content)
        open(dirname(__file__) + "/resource/" + "hencan.jpg", 'wb').write(
            requests.get("https://cdn.jsdelivr.net/gh/yzyyz1387/blogimages/hencan.jpg").content)
    print(state)
...
@ces.handle()
async def first_receive(state: T_State = State()):
    # 获取用户原始命令，如：/test
    print(state)
...
150
""")
async def get_1(state: T_State = State()):
...
（回复数字）
    """)
async def get_3(state: T_State = State()):
    if not str(state['musttime']).isdigit():
...
（回复数字）
""")
async def get_4(state: T_State = State()):
    if not str(state['gowork']).isdigit():
...
（回复数字）
""")
async def get_5(state: T_State = State()):
    if not str(state['moyu']).isdigit():
...
（回复数字）
""")
async def get_6(state: T_State = State()):
...
（回复数字）
""")
async def get_7(state: T_State = State()):
...
（回复数字）
""")
async def get_8(state: T_State = State()):
...
（回复数字）
""")
async def get_9(state: T_State = State()):
...
（回复数字）
""")
async def get_10(state: T_State = State()):
...
否
""")
async def get_11(state: T_State = State()):
...
（回复数字）
    """)
async def get_12(state: T_State = State()):
    global img, score
